...
const residentialPlotRentModel = require('../models/residentialPlotRentModel');
...
exports.postRentResidentialPlots = async (req, res) => {
  try {
    ...
    if (/^\d/.test(placePart)) {
      ...
      if (match) {
        ...
        const id = await residentialPlotRentModel.addResidentialPlotRent(req.body);
...
exports.getAllResidentialRentProperties = async (req, res) => {
  try {
    const properties = await residentialPlotRentModel.getAllResidentialPlotsRent();
    res.json({ properties });
  } catch (error) {
...
exports.filterResidentialRentProperties = async (req, res) => {
  try {
    const { state, city, pincode, min, max } = req.body;
    const prop = await residentialPlotRentModel.filterResidentialPlotsRent(state, city, pincode, min, max);
    res.json({ prop });
  } catch (error) {
...
exports.filterMapRentResidentialPlots = async (req, res) => {
  try {
