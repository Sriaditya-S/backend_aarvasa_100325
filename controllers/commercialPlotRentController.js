...
const commercialPlotRentModel = require('../models/commercialPlotRentModel');
...
exports.postRentCommercialPlots = async (req, res) => {
  try {
    ...
    if (/^\d/.test(placePart)) {
      ...
      if (match) {
        ...
        const id = await commercialPlotRentModel.addCommercialPlotRent(req.body);
...
exports.getAllCommercialRentProperties = async (req, res) => {
  try {
    const properties = await commercialPlotRentModel.getAllCommercialPlotsRent();
    res.json({ properties });
  } catch (error) {
...
exports.filterCommercialRentProperties = async (req, res) => {
  try {
    const { state, city, pincode, min, max } = req.body;
    const prop = await commercialPlotRentModel.filterCommercialPlotsRent(state, city, pincode, min, max);
    res.json({ prop });
  } catch (error) {
...
exports.filterMapRentCommercialPlots = async (req, res) => {
  try {
